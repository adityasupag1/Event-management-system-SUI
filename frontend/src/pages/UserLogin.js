...
const UserLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await API.post('/auth/user/login', { userId, password });
      login(data);
      navigate('/user/home');
    } catch (error) {
      ...
      <div className="card-container">
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-row">
            ...
            <input className="form-input" type="text" placeholder="User email" value={userId} onChange={(e) => setUserId(e.target.value)} />
          </div>
          <div className="form-row">
            <div className="form-label">Password</div>
            <input className="form-input" type="password" placeholder="User password" value={password} onChange={(e) => setPassword(e.target.value)} />
          </div>
        ...
        <p style={{textAlign: 'center', marginTop: 15}}>
          New User? <button style={{background:'none', border:'none', color:'#4472C4', cursor:'pointer', textDecoration:'underline'}} onClick={() => navigate('/user/signup')}>Sign Up</button>
        </p>
      </div>
    </div>
  );
